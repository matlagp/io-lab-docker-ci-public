...
image:
	docker build . \
		...
		--build-arg SCHEMA_VENDOR="$(SCHEMA_VENDOR)" \
		...
		-t "$(SCHEMA_VENDOR)/$(REPO_NAME):latest" \
	&& docker tag "$(SCHEMA_VENDOR)/$(REPO_NAME):latest" "$(SCHEMA_VENDOR)/$(REPO_NAME):$(TAG)"
	
push: image
	docker push "$(SCHEMA_VENDOR)/$(REPO_NAME):latest"
	docker push "$(SCHEMA_VENDOR)/$(REPO_NAME):$(TAG)"
